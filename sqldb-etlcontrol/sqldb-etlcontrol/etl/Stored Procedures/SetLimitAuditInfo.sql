...
-- =============================================
CREATE PROCEDURE [etl].[SetLimitAuditInfo] @TaskKey INT, @TaskAuditKey BIGINT, @ExecutionSequence NVARCHAR(5) AS
/*
...
The procedure is designed to run using default values if none have been assigned for this particular TaskKey in the etl.Tasks table. Ideally these global defaults should be configured in the etl.KeyValueConfig table. However, if this table is empty, then the stored procedure uses some hard-coded default values.
The only time the procedure sets a LowerLimit value in the "Pre" execution sequence is if the existing LowerLimit value on etl.Tasks is null. In this situation, it replaces the null value with the default lower limit value.
If the procedure uses defaults (whether hard-coded or from the etl.Config table), then those values will be written back to the etl.Tasks to help "seed" the incremental settings. Of course these can be adjusted later to take effect during subsequent ETL executions.

*/
SET NOCOUNT ON;
-- Only execute this proc if the TaskKey refers to an incremental action
IF EXISTS (SELECT TOP 1 TaskKey FROM etl.Task WHERE TaskKey = @TaskKey AND IsIncrementalFlag = 1 AND DisableLimitUpdate = 0)
BEGIN
	...
	BEGIN
		...
		-- Get the incremental info for this TaskKey
		SELECT
			@LimitType = LimitType
		...
		-- Set some defaults if the incremental info for this TaskKey is incomplete
		SET @TimeZoneName = COALESCE(@TimeZoneName, (SELECT TOP 1 ConfigValue FROM etl.KeyValueConfig WHERE ConfigKey = 'DefaultTimeZoneName'), 'UTC')
		SET @Offset = COALESCE(@Offset, (SELECT TOP 1 ConfigValue FROM etl.KeyValueConfig WHERE ConfigKey = 'DefaultIncrementalOffset'), 0) -- default offset is "no overlap"
		SET @OffsetUnit = COALESCE(@OffsetUnit, (SELECT TOP 1 ConfigValue FROM etl.KeyValueConfig WHERE ConfigKey = 'DefaultIncrementalOffsetUnit'), 'Day')
		SET @DefaultIncrementalEarliestDate = COALESCE(TRY_CONVERT(DATETIME, ((SELECT TOP 1 ConfigValue FROM etl.KeyValueConfig WHERE ConfigKey = 'DefaultIncrementalEarliestDate'))), '1900-01-01')
		SET @LimitType = COALESCE(@LimitType, (SELECT TOP 1 ConfigValue FROM etl.KeyValueConfig WHERE ConfigKey = 'DefaultIncrementalLimitType'), 'Date')
		...
		BEGIN
		   ...
		   ELSE IF @LimitType = 'Date' -- just the date, no time component
			  BEGIN
				 ...
				 SET @LowerLimit =
					...
					END
			  END
		   -- write the limits (and other values) back to the etl.Tasks table. 
		   UPDATE etl.Task 
		   SET 
			  LowerLimit = @LowerLimit,
			  UpperLimit = @UpperLimit,
		   ...
		   INSERT INTO etl.TaskLimitAudit (TaskAuditKey, TaskKey, ExecutionSequence, LimitAuditTimestamp, LowerLimit, UpperLimit, LimitType, Offset, OffsetUnit, TimeZoneName)
		   VALUES (@TaskAuditKey, @TaskKey, @ExecutionSequence, GETUTCDATE(), @LowerLimit, @UpperLimit, @LimitType, @Offset, @OffsetUnit, @TimeZoneName)
		END
		ELSE
